...
public class AndroidRootElement extends RootElement {
	...
	public void tap(ActionStatus status, FoundElement element, MouseDirection position) {
		...
		int offsetY = (int)(element.getBoundY() + driver.getOffsetY(rect, position));
		
		// -----
		// Fix SAMSUNG element center problem
		if (driver.deadZone != null) {
			if (rect.y >= driver.deadZone.y) {
				status.setError(-3, "Element not interactable on this device");
				return;
			} else if (rect.y + offsetY >= driver.deadZone.y) {
				offsetY = (int) (driver.deadZone.y - rect.y - 1);
			}
		}
		// -----
		
		driver.executeRequest(MobileDriverEngine.ELEMENT, element.getId(), MobileDriverEngine.TAP, (int)(element.getBoundX() + driver.getOffsetX(rect, position)) + "", offsetY + "");
	}
